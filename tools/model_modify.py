...
def surgery_loop(args, surgery_func):
    save_name = args.tar_name + '_' + (str(args.method) if args.method == "remove" else "surgery") + ".pth"
    ...
    if args.method == 'remove':
        ...
        for idx, (param_name, tar_size) in enumerate(zip(args.param_name, tar_sizes)):
            surgery_func(param_name, True, tar_size, ckpt)
            surgery_func(param_name, False, tar_size, ckpt)
    else:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser("Modify Models for finetune")
